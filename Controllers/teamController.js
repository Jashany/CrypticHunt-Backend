...
const createTeam = asyncHandler(async (req, res) => {
    ...
    const existingMembership = await Team.findOne({ "members.user": userId }).populate('members.user', 'email');
...
const joinTeam = asyncHandler(async (req, res) => {
    ...
    const existingMembership = await Team.findOne({ "members.user": userId }).populate('members.user', 'email');
    if (existingMembership) {
        return res.status(400).json({ message: 'You are already part of a team and cannot join another' });
    }
    ...
    if (team.members.length >= 3) {
        return res.status(400).json({ message: 'This team already has the maximum number of members' });
    }
